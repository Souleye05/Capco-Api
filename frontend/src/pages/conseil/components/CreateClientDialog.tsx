...
import { useCreateClientConseil } from '@/hooks/useConseil';
import { CreateClientConseilDto, StatutClientConseil, TypePartie } from '@/types/conseil';
...
const createClientSchema = z.object({
  ...
  honoraireMensuel: z.number().min(0, 'Le montant doit être positif').default(0),
  jourFacturation: z.number().min(1).max(31).default(1),
  statut: z.enum(['ACTIF', 'SUSPENDU', 'RESILIE']).default('ACTIF'),
...
export const CreateClientDialog: React.FC<CreateClientDialogProps> = ({
  ...
  const createClientMutation = useCreateClientConseil();
  ...
  const onSubmit = async (data: CreateClientFormData) => {
    try {
      const createData: CreateClientConseilDto = {
        nom: data.nom,
        type: data.type,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Créer un nouveau client pour l'assistance juridique
          </DialogDescription>
        </DialogHeader>
